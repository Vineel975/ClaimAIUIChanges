...
import { coerceClaimType, getRules } from "@/lib/rules";
...
export function PatientInfoTab({
  ...
  // Safely coerce claimType from prop (defaults to "cataract" for backward compat).
  // Then load the rules once and use the UI flags to drive visibility.
  const claimTypeSafe = coerceClaimType(claimType);
  const rules = getRules(claimTypeSafe);
  ...
  useEffect(() => {
    ...
    // Re-fetch when:
    //   - patientInfoDb isn't loaded at all, OR
    //   - it's loaded but has NO UCR section. This handles claims that were
    //     processed under an older build (before UCR existed): their cached
    //     patientInfoDb has Hospital Past History but no UCR, and without this
    //     check the UI would never call /api/patient-info-db to pick up UCR.
    const hasSections = !!patientInfoDb?.sections?.length;
    const hasUcr = !!patientInfoDb?.sections?.some((section) => {
      const name = (section.name || "").toLowerCase();
      if (name.includes("ucr")) return true;
      const firstRow = section.rows?.[0] || {};
      return Object.keys(firstRow).some((key) => key.toLowerCase().includes("ucr"));
    });
    if (hasSections && hasUcr) return; // already have UCR — nothing to do
    ...
    const renderGroupContent = (
      ...
      if (groupId === "ucrPastHistory") {
        ...
        const pastHistorySections = sections.filter((section) => {
          const sectionName = section.name.toLowerCase();
          // Never let UCR sections leak into Hospital Past History — their rows
          // share column names (claim_id, claimed_amount, approved_amount).
          if (sectionName.includes("ucr")) return false;
          if (
            ...
            if (shouldUseAccordion) {
              ...
              return (
                <Accordion
                  ...
                  type="single"
                  collapsible
                  className={`rounded-lg border px-3 transition-shadow ${isHighlighted ? "border-primary/25 bg-primary/5 shadow-sm" : "border-border bg-background"}`}
                >
                  <AccordionItem value={group.id} className="border-none">
                    <AccordionTrigger className={`py-3 text-sm font-semibold hover:no-underline ${isHighlighted ? "text-primary" : ""}`}>
    ...
    return (
      <div className="space-y-3">
        <div className="grid grid-cols-1 gap-x-3 gap-y-2 md:grid-cols-2">
          ...
          {/* Availed Accommodation — dropdown from Spectra mFacility (read-only) */}
          <div className="space-y-1">
            <div className="flex items-center justify-between gap-3">
            ...
            {facilityOptions.length > 0 ? (
              <select
                ...
                disabled
                aria-readonly="true"
                className="w-full rounded-md border border-input bg-muted/50 px-3 py-1.5 text-sm shadow-sm cursor-not-allowed focus:outline-none focus:ring-1 focus:ring-ring"
              >
  ...
  return (
    <Card className="border-border shadow-sm">
      <CardHeader>
        <CardTitle className="font-display text-base font-semibold tracking-tight">Patient Information</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {(() => {
          ...
          const lineOfTreatmentPage = (displayAnalysis?.medicalAdmissibility as { lineOfTreatmentPageNumber?: number | null } | null | undefined)?.lineOfTreatmentPageNumber;
          const treatmentPageClickable =
            typeof lineOfTreatmentPage === "number" &&
            lineOfTreatmentPage > 0 &&
            !!onScrollToPage;
          ...
          return (
            <div className="rounded-lg border border-primary/20 bg-primary/5 px-3.5 py-2.5 text-sm text-foreground/90 leading-relaxed space-y-0.5">
              <div>{line1}</div>
              ...
              {lineOfTreatment && (
                <div>
                  ...
                  {treatmentPageClickable ? (
                    <button
                      type="button"
                      onClick={() => onScrollToPage?.(lineOfTreatmentPage as number)}
                      className="text-primary underline underline-offset-2 hover:text-primary/80 cursor-pointer transition-colors"
                      title={`Go to source (page ${lineOfTreatmentPage})`}
                    >
                      {lineOfTreatment}
                    </button>
                  ) : (
              ...
              {patientInfoDb?.benefitPlanName && (
                <div><span className="font-medium">Benefit Plan Name:</span> {patientInfoDb.benefitPlanName}</div>
              )}
              {patientInfoDb?.insurerShortCode && (
                <div><span className="font-medium">Insurer Short Code:</span> {patientInfoDb.insurerShortCode}</div>
              )}
            </div>
          );
        })()}
        ...
        {/* Inadmissibility flags — shown for any disease whose rules say so.
            Currently true only for maternity (see lib/rules/maternity.ts → ui.showInadmissibilityFlags). */}
        {rules.ui.showInadmissibilityFlags && (() => {
        ...
        {(() => {
          ...
          // Drives label + ordering: maternity shows "Supporting Documents" with found items first,
          // others show "Investigation Report" with the standard layout.
          const isMaternity = claimTypeSafe === "maternity";
