...
interface MedicalAdmissibilityTabProps {
  ...
  /** Claim ID — used to fetch Claim History for the expandable section. */
  claimId?: string;
}
...
function buildConditionRows(
  ...
  for (const rule of conditionRules) {
    ...
    for (const testRule of rule.tests) {
      ...
      // Diagnosis label: prefer the document-sourced diagnosis extracted by the
      // AI (aiCondition.condition / matchedDiagnosis). Fall back to a clean
      // disease name (e.g. "Cataract") only when the document yielded nothing —
      // NOT the hardcoded rule.label which carries the "(A-scan)" test suffix.
      const documentDiagnosis =
        (aiCondition?.condition && aiCondition.condition.trim()) ||
        (selectedCondition?.condition && selectedCondition.condition.trim()) ||
        (aiCondition?.matchedDiagnosis && aiCondition.matchedDiagnosis.trim()) ||
        (selectedCondition?.matchedDiagnosis && selectedCondition.matchedDiagnosis.trim()) ||
        "";
      // Fall back to a clean disease name (without the "(A-scan)" test suffix)
      // when the document yielded no diagnosis. Uses the rule key, capitalized.
      const fallbackDiagnosis =
        rule.key.charAt(0).toUpperCase() + rule.key.slice(1);
      const displayCondition = documentDiagnosis || fallbackDiagnosis;
...
export function MedicalAdmissibilityTab({
  ...
  // ── Claim History (expandable, below Presenting Complaint) ──────────────────
  const [claimHistoryOpen, setClaimHistoryOpen] = useState(false);
  const [claimHistoryRows, setClaimHistoryRows] = useState<
    Array<Record<string, string | null>>
  >([]);
  const [claimHistoryLoading, setClaimHistoryLoading] = useState(false);
  const [claimHistoryFetched, setClaimHistoryFetched] = useState(false);

  // Fetch on first expand (lazy — don't query until the doctor opens it).
  useEffect(() => {
    if (!claimHistoryOpen || claimHistoryFetched || !claimId) return;
    let cancelled = false;
    ...
    fetch("/api/claim-history", {
      ...
      .then((data) => {
        if (cancelled) return;
        setClaimHistoryRows(Array.isArray(data?.rows) ? data.rows : []);
        setClaimHistoryFetched(true);
      })
      .catch(() => {
        if (!cancelled) setClaimHistoryFetched(true);
      })
      .finally(() => {
        if (!cancelled) setClaimHistoryLoading(false);
      });
    return () => {
      cancelled = true;
    };
  }, [claimHistoryOpen, claimHistoryFetched, claimId]);
          ...
          <div className="space-y-4">
              ...
              {medicalAdmissibility.doctorNotes && (
                <div className="space-y-2">
                  ...
                  >
                    {medicalAdmissibility.doctorNotes
                      // Tightly pack the notes: strip trailing spaces per line and
                      // collapse ALL blank lines (any run of 2+ newlines becomes a
                      // single newline) so lines sit directly under one another with
                          ...
                          <table className="w-full border-collapse text-xs">
                            <thead>
                              <tr className="bg-primary/10 text-left text-foreground">
                                ...
                                <th className="whitespace-nowrap px-2 py-1">Relationship</th>
                                <th className="whitespace-nowrap px-2 py-1">DOA–DOD</th>
                                <th className="whitespace-nowrap px-2 py-1">Hospital Name</th>
                                <th className="whitespace-nowrap px-2 py-1">Diagnosis</th>
                                <th className="whitespace-nowrap px-2 py-1">Service Type</th>
                                <th className="whitespace-nowrap px-2 py-1">Claim Type</th>
                                <th className="whitespace-nowrap px-2 py-1">Claim Sub Type</th>
                                <th className="whitespace-nowrap px-2 py-1">Request Type</th>
                                <th className="whitespace-nowrap px-2 py-1">Internal Claim Status</th>
                                <th className="whitespace-nowrap px-2 py-1">Claimed Amount</th>
                                <th className="whitespace-nowrap px-2 py-1">Settled Amount</th>
                              </tr>
                            </thead>
                            <tbody>
                              {claimHistoryRows.map((row, idx) => {
                                const doa = row.dateOfAdmission
                                  ? new Date(row.dateOfAdmission).toLocaleDateString("en-IN")
                                  : "";
                                const dod = row.dateOfDischarge
                                ...
                                return (
                                  <tr key={idx} className="border-t border-border odd:bg-card even:bg-muted/30">
                                    ...
                                    <td className="whitespace-nowrap px-2 py-1">{row.uhidNo ?? ""}</td>
                                    <td className="whitespace-nowrap px-2 py-1">{row.patientName ?? ""}</td>
                                    <td className="whitespace-nowrap px-2 py-1">{row.relationship ?? ""}</td>
                                    <td className="whitespace-nowrap px-2 py-1">{doa}{dod ? ` To ${dod}` : ""}</td>
                                    <td className="px-2 py-1">{row.hospitalName ?? ""}</td>
                                    <td className="px-2 py-1">{row.diagnosis ?? ""}</td>
                                    <td className="whitespace-nowrap px-2 py-1">{row.serviceType ?? ""}</td>
                                    <td className="whitespace-nowrap px-2 py-1">{row.claimType ?? ""}</td>
                                    <td className="whitespace-nowrap px-2 py-1">{row.serviceSubType ?? ""}</td>
                                    <td className="whitespace-nowrap px-2 py-1">{row.requestType ?? ""}</td>
                                    <td className="whitespace-nowrap px-2 py-1">{row.internalStage ?? ""}</td>
                                    <td className="whitespace-nowrap px-2 py-1 text-right">{row.claimedAmount ?? ""}</td>
                                    <td className="whitespace-nowrap px-2 py-1 text-right">{row.settledAmount ?? ""}</td>
                                  </tr>
                                );
                              })}
                            </tbody>
